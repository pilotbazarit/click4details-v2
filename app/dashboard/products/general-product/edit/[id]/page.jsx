...
const AttributeField = ({ control, index, watch, features, remove, showRemoveButton, setValue, errors }) => {
    ...
    const currentAttributeValue = watch(`attributes.${index}.attribute_value_id`);
    ...
    useEffect(() => {
        const fetchFeatureItems = async () => {
            if (selectedFeatureId) {
                ...
                // Only reset attribute value if NOT initial load (user manually changed feature)
                if (!isInitialLoad) {
...
const GeneralProductEdit = (productId) => {
    ...
    const [existingImageData, setExistingImageData] = useState(null); // Store existing image data with IDs
    ...
    // State to track if initial APIs are loaded
    const [isInitialDataLoaded, setIsInitialDataLoaded] = useState(false);
    const [initialModelId, setInitialModelId] = useState(null);
    const [initialPackageId, setInitialPackageId] = useState(null);
    const [initialOutletId, setInitialOutletId] = useState(null);
    ...
    // Fetch product details only after initial data is loaded
    useEffect(() => {
        if (!isInitialDataLoaded || !id) return;

        const fetchProductDetails = async () => {
            try {
                const response = await GeneralProductService.Queries.getGeneralProductDetailById(id);
                ...
                if (response.status == "success") {
                    ...
                    const brandId = data?.brand ? data.brand.md_id : null;
                    const modelId = data?.p_model_id ?? "";
                    const packageId = data?.p_package_id ? data.p_package_id : null;

                    //   setValue("p_type_id", data?.p_type_id || null);
                    setValue("p_type_id", String(data?.p_type_id ?? ""));

                    // Fetch categories for the product type and set category history
                    if (data?.p_type_id && data?.p_category_id) {
                        // Set the category value
                        setValue("category", data?.p_category_id);

                        // Build category history - fetch the category path from API
                        const buildCategoryPath = async (categoryId) => {
                            try {
                                const response = await CategoryService.Queries.getCategories({
                                    _page: 1,
                                    _perPage: 5000,
                                    _parent_id: data.p_type_id,
                                });

                                if (response.status === "success") {
                                    const categories = response?.data?.data;
                                    const selectedCategory = categories.find(cat => cat.c_id === categoryId);

                                    if (selectedCategory) {
                                        setCategoryHistory([{
                                            c_id: selectedCategory.c_id,
                                            c_name: selectedCategory.c_name
                                        }]);
                                        // Fetch subcategories for the selected category
                                        fetchCategories(categoryId);
                                    } else {
                                        // Category not found at root level, just fetch from product type
                                        fetchCategories(data.p_type_id);
                                    }
                                }
                            } catch (error) {
                        ...
                        buildCategoryPath(data.p_category_id);
                    }

                    setValue("productName", data.p_name || "");
                    setValue("brand", brandId);
                    // Store model ID to set after models are fetched
                    if (modelId) {
                        setInitialModelId(modelId);
                    }

                    // Store package ID to set after packages are fetched
                    if (packageId) {
                        setInitialPackageId(packageId);
                    }

                    const shopId = data?.shop ? data.shop.s_id : null;
                    const outletId = data?.outlet ? data.outlet.uo_id : null;
                    // const countryId = data?.country ? data.country.c_id : null;
                    const countryId = data?.location ? data.location.l_country_id : null;
                    const locationId = data?.location ? data.location.l_id : null;

                    setValue("shop", shopId);
                    setValue("code", data?.code || "");
                    setValue("country", countryId);

                    // Store outlet ID to set after outlets are fetched
                    if (shopId && outletId) {
                        setInitialOutletId(outletId);
                    }

                    // Store location ID to set after locations are fetched
                    if (countryId && locationId) {
                        setInitialLocationId(locationId);
                    }

                    setValue("serialNumber", data?.p_serial_no || "");
                    setValue("videoLink", data?.p_video_link || "");

                    setValue("pbl_description", data?.p_description ? data?.p_description?.pbl : "");
                    setValue("special_description", data?.p_description ? data?.p_description?.meta : "");
                    setValue("user_description", data?.p_description ? data?.p_description?.user : "");


                    // Set image preview from existing data
                    if (data?.p_primary_image && data?.p_primary_image?.secure_url) {
                        setImagePreview(data.p_primary_image.secure_url);
                        setValue("image", data.p_primary_image.secure_url);
                        // Store existing image data with ID
                        setExistingImageData({
                            public_id: data.p_primary_image.public_id,
                            url: data.p_primary_image.secure_url
                        });
                    }

                    // Set images preview from existing data
                    if (data?.p_images && Array.isArray(data?.p_images) && data?.p_images.length > 0) {
                        const imageUrls = data.p_images.map(img => img.secure_url || img.url);
                        setImagesPreview(imageUrls);
                        setValue("images", data.p_images); // Store full image objects with IDs
                    }

                    // Set prices from API data
                    if (data?.prices && Array.isArray(data.prices) && data.prices.length > 0) {
                        const pricesData = data.prices.map(price => ({
                            purchase_price: price.pp_purchase_price || "",
                            regular_price: price.pp_regular_price || "",
                            discount: price.pp_discount_percentage || "",
                            discount_price: price.pp_discount_price || "",
                            asking_price: price.pp_asking_price || "",
                            fixed_price: price.pp_fixed_price || "",
                            unit_id: price.pp_unit_id || "",
                            unit_count: price.pp_unit_count || "",
                            wholesale_price: price.pp_wholesale_price || "",
                            min_unit_wholesale: price.pp_minimum_wholesale_qty || ""
                        }));
                    ...
                    if (data?.attributes && Array.isArray(data.attributes) && data.attributes.length > 0) {
                        const attributesData = data.attributes.map(attr => ({
                            attribute_id: attr.pam_attr_id || "",
                            attribute_value_id: attr.pam_attr_value_id || ""
                        }));
                        setValue("attributes", attributesData);
                    }

                    // Set extra information from API data
                    if (data?.extended_data && Array.isArray(data.extended_data) && data.extended_data.length > 0) {
                        const extraInfoData = data.extended_data.map(extra => ({
                            title: extra.ed_entity_key || "",
                            value: extra.ed_entity_value || ""
                        }));
                        setValue("extraInfo", extraInfoData);
                    }
    ...
    // Load all initial data before fetching product details
    useEffect(() => {
    ...
    // Set initial model value after model data is loaded
    useEffect(() => {
        if (initialModelId && modelData.length > 0) {
            const modelExists = modelData.find(m => m.value === initialModelId);
            if (modelExists) {
                setValue("model", initialModelId);
                setInitialModelId(null); // Clear after setting
            }
        }
    }, [modelData, initialModelId, setValue]);
    ...
    // Set initial package value after package data is loaded
    useEffect(() => {
        if (initialPackageId && packageData.length > 0) {
            const packageExists = packageData.find(p => p.value === initialPackageId);
            if (packageExists) {
                setValue("p_package_id", initialPackageId);
                setInitialPackageId(null); // Clear after setting
            }
        }
    }, [packageData, initialPackageId, setValue]);
    ...
    // Set initial location value after location data is loaded
    useEffect(() => {
        if (initialLocationId && locationData.length > 0) {
            const locationExists = locationData.find(l => l.value === initialLocationId);
            if (locationExists) {
                setValue("location", initialLocationId);
                setInitialLocationId(null); // Clear after setting
            }
        }
    }, [locationData, initialLocationId, setValue]);
    ...
    // Set initial outlet value after outlet data is loaded
    useEffect(() => {
        if (initialOutletId && outletData.length > 0) {
            const outletExists = outletData.find(o => o.value === initialOutletId);
            if (outletExists) {
                setValue("outlet", initialOutletId);
                setInitialOutletId(null); // Clear after setting
            }
        }
    }, [outletData, initialOutletId, setValue]);
    ...
    const onSubmit = async (data) => {
        ...
        // Handle primary image - only append if it's a new file (not a URL)
        if (data.image && typeof data.image !== 'string') {
            formData.append("p_primary_image", data.image);
        }

        // Handle images array - only append new files (not existing URLs/objects)
        if (data.images && data.images.length > 0) {
            data.images.forEach((file) => {
                // Only append if it's a File object (newly uploaded)
        ...
        if (removedImageIds.length > 0) {
            removedImageIds.forEach((imageId) => {
                formData.append("p_remove_image[]", imageId);
            });
        }
        ...
        try {
            ...
            const response = await GeneralProductService.Commands.updateGeneralProduct(id, formData);
    ...
    const handleRemoveImage = () => {
        if (imagePreview) {
            // Check if it's an existing image (not a newly uploaded blob)
            if (existingImageData && existingImageData.url === imagePreview) {
                // Add to removed image IDs
                setRemovedImageIds(prev => [...prev, existingImageData.public_id]);
            } else {
                // It's a newly uploaded image, just revoke the blob URL
    ...
    console.log("setRemovedImageIds", removedImageIds);
    ...
    const handleRemoveImages = (index) => {
        ...
        // Check if it's an existing image (has public_id)
        if (imageToRemove && typeof imageToRemove === 'object' && imageToRemove.public_id) {
            // Add to removed image IDs
                ...
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 p-4 bg-white shadow-sm rounded-lg mb-6 border border-gray-200 mt-4">
                    <div>
                        <h2 className="text-2xl font-bold text-gray-800">
                            ✏️ Edit General Product
                        </h2>
                        <p className="text-sm text-gray-500 mt-1">
                            Update existing general product details.
                        </p>
                    </div>
...
export default function Page(productId) {
    return (
        <>
            <GeneralProductEdit productId={productId} />
        </>
    );
}
