...
const getAuctionTypeLabel = (value) => {
    const matchedOption = auctionTypeOptions.find((option) => option.value === value);
    if (matchedOption) return matchedOption.label;
    return value ? String(value).toUpperCase() : "";
};

const getGoogleDocLink = (data) => {
    const rawVideoData = data?.v_video;

    if (rawVideoData && typeof rawVideoData === "object") {
        return rawVideoData?.gdoc ? String(rawVideoData.gdoc).trim() : "";
    }
        ...
        if (trimmedVideo.startswith("{") && trimmedVideo.endswith("}")) {
            try {
                ...
                if (parsedVideo && typeof parsedVideo === "object") {
                    return parsedVideo?.gdoc ? String(parsedVideo.gdoc).trim() : "";
                }
            } catch (error) {
                console.log("Failed to parse Google doc link:", error);
            }
        }
    }

    return data?.v_video_gdoc ? String(data.v_video_gdoc).trim() : "";
};

const getDisplayVideoLink = (data) => {
    const rawVideoData = data?.v_video;

    if (rawVideoData && typeof rawVideoData === "object") {
        if (rawVideoData?.user) return String(rawVideoData.user).trim();
        if (rawVideoData?.pbl) return String(rawVideoData.pbl).trim();
        if (rawVideoData?.gdoc) return String(rawVideoData.gdoc).trim();
    }
    ...
    if (typeof rawVideoData === "string") {
        ...
        if (trimmedVideo.startsWith("{") && trimmedVideo.endsWith("}")) {
            try {
                const parsedVideo = JSON.parse(trimmedVideo);
                if (parsedVideo && typeof parsedVideo === "object") {
                    if (parsedVideo?.user) return String(parsedVideo.user).trim();
                    if (parsedVideo?.pbl) return String(parsedVideo.pbl).trim();
                    if (parsedVideo?.gdoc) return String(parsedVideo.gdoc).trim();
                }
            } catch (error) {
                return trimmedVideo;
            }
    ...
    if (data?.v_video_user) return String(data.v_video_user).trim();
    if (data?.v_video_pbl) return String(data.v_video_pbl).trim();
...
const ViewProductForm = ({ productId }) => {
    ...
    const handleCategoryClick = async (category) => {
        ...
        try {
            const response = await CategoryService.Queries.getCategories({
                _page: 1,
                _perPage: 5000,
                _parent_id: category.c_id,
            });

            const children = response?.status === "success" ? (response?.data?.data || []) : [];
            if (children.length > 0) {
                const mapped = children.map((item) => ({ c_id: item.c_id, c_name: item.c_name }));
    ...
    const handleList = async () => {
        router.push("/dashboard/requested-product/");
    }
    ...
    const fetchProductDetails = async () => {
        try {
            ...
            if (res.status === 'success') {
                ...
                setValue('vp_pbl_price_status', data?.vehicle_db_price?.vp_pbl_price_status);
                ...
                setValue('v_video_gdoc', getGoogleDocLink(data));
                ...
                setValue('v_video', getDisplayVideoLink(data));
                setValue('v_is_saleBy_pbl', data.v_is_saleBy_pbl == 1);

                // Set seller info details
                setSellerDetails({
                    shopName: data.v_shop_name || data?.vehicle_shop?.s_title || "",
                    shopUserName: data.v_shop_user_name || data?.vehicle_shop?.user?.name || data?.user?.name || "",
                    shopUserPhone: data.v_shop_user_phone || data?.vehicle_shop?.user?.phone || data?.user?.phone || "",
                    userPhone: data?.user?.phone || data?.v_shop_user_phone || "",
                    userName: data?.user?.name || data?.v_shop_user_name || ""
                });
    ...
    const onSubmit = async (data) => {
        ...
        const payload = {
            _is_approved: 1,
            ...data,
            v_seller_info: sellerInfoPayload,
            v_show_seller_mobile: data.v_show_seller_mobile ? 1 : 0,
            vp_pbl_additional_amount: data?.vp_pbl_additional_amount ?? "",
            vp_user_variable_price: data?.vp_user_variable_price ?? "",
            vp_user_costing_price: data?.vp_user_costing_price ?? "",
            vp_pbl_price_status: data?.vp_pbl_price_status ?? "",
            vp_pbl_asking_price: data?.vp_pbl_asking_price ?? "",
        };

        try {
            setLoading(true);
            const response = await VehicleService.Commands.approvedVehicle(productId, payload);

            if (response.status === 'success') {
                toast.success(isEditing ? "Vehicle Updated Successfully!" : "Vehicle Approved & Updated Successfully!");
                router.push("/dashboard/requested-product/");
            }
        } catch (error) {
            toast.error(error?.response?.data?.message || "Vehicle update/approval failed!");
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
                    ...
                    <div className="w-full md:p-10 p-4">
                        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 p-4 bg-white shadow-sm rounded-lg mb-6 border border-gray-200">
                            ...
                            <div className="flex flex-wrap items-center gap-2.5 text-right">
                                ...
                                <Button
                                    type="button"
                                    onClick={() => router.push(`/dashboard/products/vehicle/edit/${productId}`)}
                                    variant="outline"
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                                            ...
                                            <div>
                                                ...
                                                <div className="relative" ref={categoryDropdownRef}>
                                                    <div
                                                        onClick={() => { if (!isReadOnly && selectedProductTypeId) setIsCategoryListVisible(!isCategoryListVisible); }}
                                                        className={`flex justify-between items-center border rounded-lg p-2 bg-white shadow-sm ${!isReadOnly && selectedProductTypeId ? "cursor-pointer" : "cursor-not-allowed bg-gray-100"}`}
                                                    >
                                                    ...
                                                    {isCategoryListVisible && selectedProductTypeId && !isReadOnly && (
                                                ...
                                                <Controller
                                                    ...
                                                    render={({ field }) => (
                                                        <Select
                                                            ...
                                                            isDisabled={isReadOnly || !selectedShopId}
                                                            className="basic-single"
                                                            classNamePrefix="select"
                                                        />
                                                    )}
                                                ...
                                                <Controller
                                                    ...
                                                    render={({ field }) => (
                                                        <Select
                                                            ...
                                                            isDisabled={isReadOnly || !selectedCountryId}
                                                            className="basic-single"
                                                            classNamePrefix="select"
                                                        />
                                                    )}
                                ...
                                <fieldset disabled={isReadOnly} className="contents">
                                    ...
                                    <div className="px-4">
                                            ...
                                            ((user?.user_mode === 'supreme') || (user?.user_mode === 'admin') || (user?.user_mode === 'pbl')) && (
                                                <>
                                                    ...
                                                    <div className="grid grid-cols-4 gap-4 mb-4">
                                                        ...
                                                        <div className="mb-2">
                                                            <label className="text-base font-medium" htmlFor="vp_pbl_additional_amount">
                                                        ...
                                                        <div className="mb-2">
                                                            <label className="text-base font-medium" htmlFor="vp_pbl_asking_price">
                                                                PBL Asking Price
                                                            </label>
                                                            <Input
                                                        ...
                                                        <div>
                                                            <label className="text-base font-medium" htmlFor="vp_pbl_price_status">
                                                                PBL Price Negotiation
                                                            </label>
                                                            <select
                                                                id="vp_pbl_price_status"
                                                                name="vp_pbl_price_status"
                                                                disabled={isReadOnly}
                                                                className="outline-none py-2 px-3 rounded border w-full disabled:bg-gray-50"
                                                                {...register("vp_pbl_price_status")}
                                                            >
                                                        ...
                                                        <div className="mb-2">
                                                            <label className="text-base font-medium" htmlFor="pbl_partner_code">
                                                                PBL Partner Code
                                                            </label>
                                                            <Input
                                                                id="pbl_partner_code"
                                                        ...
                                                        <div className="mb-2">
                                                            <label className="text-base font-medium" htmlFor="v_video_gdoc">
                                                                Google link (Pic)
                                                            </label>
                                                            <Input
                                                                id="v_video_gdoc"
                                                        ...
                                                        <div className="mb-2">
                                                            ...
                                                            <Input
                                                                id="v_auction_type"
                                                                name="v_auction_type"
                                                                value={getAuctionTypeLabel(selectedAuctionType)}
                                                                placeholder="No Auction Type"
                                                                readOnly
                                                                disabled={isReadOnly}
                                                            />
                                                        </div>
                                        ...
                                        <div className="mb-3 mt-6">
                                            <h4 className="text-sm font-semibold text-gray-800 mb-1">Seller Information</h4>
                                            <div className="flex w-20 h-0.5">
                                                <div className="w-1/2 bg-green-500"></div>
                                                <div className="w-1/2 bg-gray-500/20"></div>
                                            </div>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4 bg-gray-50 p-4 rounded-lg border border-gray-200">
                                            <div>
                                                <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider block mb-1">Shop / Showroom</label>
                                                <p className="text-sm font-semibold text-gray-800">{sellerDetails.shopName || "N/A"}</p>
                                            </div>
                                            <div>
                                                <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider block mb-1">Seller Name</label>
                                                <p className="text-sm font-semibold text-gray-800">{sellerDetails.shopUserName || sellerDetails.userName || "N/A"}</p>
                                            </div>
                                            <div>
                                                <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider block mb-1">Seller Mobile Number</label>
                                                <p className="text-sm font-semibold text-gray-800">{sellerDetails.shopUserPhone || sellerDetails.userPhone || "N/A"}</p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="mt-4 flex items-center mb-3">
                                            ...
                                            <label htmlFor="v_show_seller_mobile" className="text-sm font-medium text-gray-700">
                                                Show Seller Mobile Number on Public Listing
                                            </label>
                                        </div>

                                        {(watch("v_show_seller_mobile") || sellerInfoRows.some(r => r.name || r.phone)) && (
                                            <div className="mt-3 p-4 bg-white rounded-lg border border-gray-200">
                                                <h5 className="text-sm font-semibold text-gray-800 mb-3">Additional Seller Contacts</h5>
                                                ...
                                                {!isReadOnly && (
                                                    <button
                                                        ...
                                                        className="mt-2 px-3 py-1.5 text-sm text-blue-600 border border-blue-300 rounded hover:bg-blue-50"
                                                    >
                                                        + Add Seller Contact
                                                    </button>
                                                )}
        ...
        <span className="text-sm font-semibold text-gray-600 mb-1">{hotlineText}</span>
                                ...
                                {
                                    ((user?.user_mode === 'supreme') || (user?.user_mode === 'admin') || (user?.user_mode === 'pbl')) && (
                                        <div className="flex justify-center gap-3 mt-6 p-4 border-t">
                                            {isEditing ? (
                                                <button
                                                    type="submit"
                                                    className="bg-emerald-600 hover:bg-emerald-700 text-white px-8 py-2.5 rounded-lg font-medium shadow-md transition"
                                                >
                                                    {staged === "review" ? "Approve & Update Vehicle" : "Save Changes"}
                                                </button>
                                            ) : (
                                                <>
                                                    {staged === "review" && (
                                                        <button
                                                            type="submit"
                                                            className="bg-green-600 hover:bg-green-700 text-white px-8 py-2.5 rounded-lg font-medium shadow-md transition"
                                                        >
                                                            Approve
                                                        </button>
                                                    )}
