...
import { GitCompare, MapPin, ReceiptText, Share2, PhoneOutgoing, ShoppingCart, Copy, Edit, Clock, ChevronDown, X, Star, Palette, Loader2, Plus } from "lucide-react"
...
const ProductCard = ({ product, parsedUser = null, sourceParam = null }) => {
  ...
  //   "https://res.cloudinary.com/pilotbazar/image/upload/vehicles/6BM29EuNbGBWwi51Z514ChHfLTLcocKGyD2QJLnv.jpg";
  const isOwnerShopPage = pathname === "/my-shop/" || pathname === "/company-shop/";
  const rawDisplayVehiclePrice = isOwnerShopPage
    ? displayVehiclePrice?.user_price
    : displayVehiclePrice?.pbl_price;
  const displayCurrency = getProductCardFirstValue(displayVehicleDbPrice?.vp_currency);
  const displayPriceText =
    rawDisplayVehiclePrice === "Call for Price"
      ? "Call for Price"
      : `${displayCurrency ? `${displayCurrency}. ` : ""}${formatPrice(rawDisplayVehiclePrice)}`;
  const displayPriceStatus =
    pathname !== "/pb-home/"
      ? displayVehicleDbPrice?.vp_pbl_price_status
      : displayVehicleDbPrice?.vp_user_price_status;
  const displayPriceStatusLabel = displayPriceStatus
    ? `${String(displayPriceStatus).charAt(0).toUpperCase()}${String(displayPriceStatus).slice(1)}`
    : "Price available";


  // const cardLocationLabel = [
  //   displayedLocationName,
  //   shouldShowOutletName ? displayedOutletName : "",
  // ]
  //   .filter(Boolean)
  //   .join(" · ") || "Location unavailable";


  const cardAvailabilityClassName =
    selectedAvailabilityStatus === "available"
      ? "border-emerald-200 bg-emerald-50 text-emerald-700"
      : selectedAvailabilityStatus === "dealer_boock"
        ? "border-amber-200 bg-amber-50 text-amber-700"
        : selectedAvailabilityStatus === "hold"
          ? "border-blue-200 bg-blue-50 text-blue-700"
          : "border-rose-200 bg-rose-50 text-rose-700";
  const cardRegistrationValue = product?.v_grade_name
    ? `${product.v_grade_name} (${[product?.v_int_grade_name, product?.v_ext_grade_name].filter(Boolean).join(" ") || "N/A"})`
  ...
  return (
    <div className="relative h-full group">
      <article className="relative flex h-full flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-blue-200 hover:shadow-xl">
        {/* Product image */}
        <div className="relative aspect-[4/3] overflow-hidden bg-slate-100">
          <Link href={href} target="_blank" className="block h-full">
            <img
              src={productImageUrl}
              alt={product?.v_title || "Vehicle"}
          ...
          <div className="absolute left-3 top-3 z-10 flex flex-wrap items-center gap-2">
            {product?.v_urgent_sale === "1" && (
              <span className="rounded-full bg-rose-500 px-3 py-1 text-[11px] font-extrabold tracking-wide text-white shadow-lg">
                URGENT SALE
              </span>
            )}
            <GiftBadge
              userGift={product?.user_gift}
          ...
          <button
            ...
            aria-label="Copy vehicle code"
            className="absolute bottom-3 left-3 z-10 inline-flex items-center gap-1.5 rounded-lg bg-black/55 px-2.5 py-1.5 text-xs font-semibold text-white backdrop-blur-sm transition hover:bg-black/75"
          >
            <Copy className="h-3.5 w-3.5" />
            {copied ? "Copied" : product?.v_code || "Vehicle code"}
          </button>

          <div className="absolute bottom-3 right-3 z-10 rounded-full border border-white/30 bg-white/15 px-2.5 py-1 text-[11px] font-semibold text-white backdrop-blur-sm">
        ...
        <div className="flex flex-1 flex-col p-4">
          {/* Title */}
          <Link href={`/product/${product?.v_id}`} target="_blank" className="block">
            <h3 className="min-h-[48px] line-clamp-2 text-base font-bold leading-6 text-blue-600 transition-colors hover:text-blue-600">
              {product?.v_title || "Untitled product"}
            </h3>
          </Link>
          ...
          <div className="mt-3 flex items-end justify-between gap-3">
            <div className="min-w-0">
              <p className="truncate text-2xl font-extrabold tracking-tight text-slate-950">
                {displayPriceText}
              </p>
              <p className="mt-1 text-xs font-medium text-slate-500">
                {displayPriceStatusLabel}
              </p>
            </div>

            {displayVehiclePrice?.price_drop_type === "drop" && displayVehiclePrice?.price_drop_percentage > 0 && (
              <span className="shrink-0 rounded-full border border-emerald-200 bg-emerald-50 px-2.5 py-1 text-[11px] font-bold text-emerald-700">
                ↓ {displayVehiclePrice.price_drop_percentage}%
              </span>
            )}
            {displayVehiclePrice?.price_drop_type === "increase" && displayVehiclePrice?.price_drop_percentage > 0 && (
          ...
          {displayVehiclePrice?.is_price_validity_expired && (
            <span className="mt-2 inline-flex w-fit items-center rounded-full border border-rose-200 bg-rose-50 px-2.5 py-1 text-[11px] font-bold text-rose-700">
              Price validity expired
            </span>
          )}

          {/* Key specifications */}
          <div className="mt-4 grid grid-cols-3 divide-x divide-slate-200 rounded-2xl border border-slate-100 bg-slate-50 py-3">
            <div className="min-w-0 px-2 text-center">
              <p className="text-[11px] font-medium text-slate-500">Condition</p>
            ...
            <div className="min-w-0 px-2 text-center">
              <p className="text-[11px] font-medium text-slate-500">
                {product?.v_grade_name ? "Point" : "Registration"}
              </p>
              <p className="mt-1 truncate text-sm font-bold text-slate-800" title={cardRegistrationValue}>
                {cardRegistrationValue}
              </p>
            </div>
            <div className="min-w-0 px-2 text-center">
              <p className="text-[11px] font-medium text-slate-500">Mileage</p>
              <p className="mt-1 truncate text-sm font-bold text-slate-800">
          ...
          {/* Location, color, rating and age */}
          <div className="mt-3 flex items-center justify-between gap-2 text-xs">
            <span
              className="inline-flex min-w-0 items-center gap-1.5 truncate font-medium text-slate-500"
              title={displayedOutletName || cardLocationLabel}
            >
              <MapPin className="h-3.5 w-3.5 shrink-0 text-blue-500" />
              <span className="truncate">{cardLocationLabel}</span>
              {displayedOutletId && pathname !== "/pb-home/" && (
                <span className="shrink-0 text-slate-400">#{displayedOutletId}</span>
              )}
            </span>

            <span className={`shrink-0 rounded-full border px-2.5 py-1 font-bold ${cardAvailabilityClassName}`}>
              {getAvailabilityStatusLabel(selectedAvailabilityStatus)}
            </span>
          </div>

          <div className="mt-3 flex min-h-7 items-center justify-between border-t border-slate-100 pt-3">
            <span className="inline-flex items-center gap-1.5 text-xs font-medium text-slate-500" title={product?.v_color_name || "Color not available"}>
              <Palette className="h-4 w-4 text-fuchsia-500" />
              <span className="max-w-[90px] truncate">{product?.v_color_name || "Color N/A"}</span>
            </span>

            <div className="flex items-center gap-3">
              {/* {!isMyShop && !isCompanyShop && formattedUserRating && (
                <span className="inline-flex items-center gap-1 text-xs font-bold text-slate-700" title={`Importer rating ${formattedUserRating}`}>
              ...
              <div className="relative">
                <button
                  ...
                  className="inline-flex items-center gap-1 rounded-full px-1.5 py-1 text-slate-400 transition hover:bg-slate-100 hover:text-slate-700"
                  aria-label="Show product age"
                  title="Show product age"
                >
                  <Clock className="h-4 w-4" />
                </button>
                {showTooltip && (
                  <span className="absolute bottom-full right-0 z-20 mb-2 whitespace-nowrap rounded-lg bg-slate-900 px-2.5 py-1.5 text-[11px] font-medium text-white shadow-lg">
            ...
            <div className="flex gap-2">
              {!isMyShop && !isCompanyShop && (
                <button
                  ...
                  title="Add to cart"
                  className="inline-flex min-h-11 flex-1 items-center justify-center gap-2 rounded-xl bg-blue-600 px-3 text-sm font-bold text-white shadow-sm transition hover:bg-blue-700 hover:shadow-md active:scale-[.98]"
                >
              ...
              {!isMyShop && !isCompanyShop && Number(parsedUser?.id) !== Number(product?.v_user_id) && (
                <button
                  type="button"
                  onClick={handleChatOpen}
                  title="Make an offer"
                  className="inline-flex min-h-11 flex-1 items-center justify-center gap-2 rounded-xl border border-rose-200 bg-rose-50 px-3 text-sm font-bold text-rose-700 transition hover:border-rose-300 hover:bg-rose-100 active:scale-[.98]"
                >
            ...
            <div className="mt-2 flex gap-2">
              {(isFilterProductPage || isSearchResultsPage || pathname === "/pb-home/" || pathname === "/" || sourceParam === "pb-home" || sourceParam === "home") && (
                <button
                  ...
                  title="Clone"
                  aria-label="Clone"
                  className="inline-flex h-10 flex-1 items-center justify-center gap-1.5 rounded-xl border border-blue-200 bg-blue-50 px-2 text-xs font-semibold text-slate-600 transition hover:border-blue-300 hover:bg-blue-50 hover:text-blue-700"
                >
                  <Copy className="h-4 w-4" />
                  {/* <span className="hidden 2xl:inline">Clone</span> */}
                </button>
              )}
              ...
              <button
                ...
                title="Share"
                aria-label="Share"
                className="inline-flex h-10 flex-1 items-center justify-center gap-1.5 rounded-xl border border-pink-200 bg-pink-50 px-2 text-xs font-semibold text-slate-600 transition hover:border-blue-300 hover:bg-blue-50 hover:text-blue-700"
              >
              ...
              {!isMyShop && !isCompanyShop && (
                <button
                  ...
                  className="inline-flex h-10 flex-1 items-center justify-center gap-1.5 rounded-xl border border-orange-200 bg-orange-50 px-2 text-xs font-semibold text-slate-600 transition hover:border-violet-300 hover:bg-violet-50 hover:text-violet-700"
                >
              ...
              {(isFilterProductPage || isSearchResultsPage || pathname === "/pb-home/" || pathname === "/" || sourceParam === "pb-home" || sourceParam === "home") && (
                <button
                  type="button"
                  onClick={() => {
                    const phoneNumber = "+8801407054400".replace("+", "");
                  ...
                  className="inline-flex h-10 flex-1 items-center justify-center gap-1.5 rounded-xl border border-emerald-200 bg-emerald-50 px-2 text-xs font-semibold text-emerald-700 transition hover:border-emerald-300 hover:bg-emerald-100"
                >
              ...
              <button
                ...
                className={`inline-flex h-10 flex-1 items-center justify-center gap-1.5 rounded-xl border px-2 text-xs font-semibold transition ${isInCompare(product?.v_id)
                  ? "border-cyan-600 bg-cyan-600 text-white"
                  : "border-cyan-200 bg-cyan-50 text-slate-600 hover:border-cyan-300 hover:bg-cyan-50 hover:text-cyan-700"
                  }`}
              >
              ...
              {(pathname === "/my-shop/" || pathname === "/company-shop/") && (
                <button
                  ...
                  className="inline-flex h-10 flex-1 items-center justify-center gap-1.5 rounded-xl border border-slate-200 bg-white px-2 text-xs font-semibold text-slate-600 transition hover:border-fuchsia-300 hover:bg-fuchsia-50 hover:text-fuchsia-700"
                >
      ...
      <Dialog open={chatConfirmOpen} onOpenChange={setChatConfirmOpen}>
        <DialogContent className="max-w-2xl">
          ...
          <div className="text-sm text-gray-600 space-y-4 leading-relaxed max-h-[60vh] overflow-y-auto pr-4">
            ...
            <div>
              <strong>৪। গুরুত্বপূর্ণ বিষয় :</strong>
              <p className="mt-1">
