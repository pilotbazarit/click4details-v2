...
export default function Theme1Messages({
  editorHeight
}: {
  editorHeight: number;
}) {
  ...
  return (
    <div className='overflow-auto relative scrollbar' ref={mainChatContainer}>
      {!isGuest && !actAsIpChat ? (
        fetched ? (
          messages?.length === 0 ? (
            <div className='flex flex-wrap justify-center gap-2 items-center mt-10'>
              {templateMessages
                ?.filter((template) => template?.showFirst)
                ?.toReversed()
                ?.map((template) => (
                  <button
                    key={template?.id}
                    onClick={() => handleSendTemplateMessage(template)}
                    className='bg-blue-600 hover:bg-blue-700 text-white px-3 py-2 rounded-[5px]'
                  >
                    {template?.question}
                  </button>
                ))}
