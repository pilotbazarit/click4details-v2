...
export default function Theme2TicketEditor({
  handleSubmit,
  subject,
  setSubject,
  description,
  ...
  return (
    <form onSubmit={handleSubmit} className='mt-4'>
            ...
            {screenshots?.map((image: any, index) => (
              <div key={index} className='relative'>
                <button
                  ...
                  type='button'
                  className='absolute top-4 right-2 text-red-600 bg-white hover:bg-red-100 w-8 h-8 cursor-pointer rounded-full'
                >
                  <Trash2 className='w-5 h-5 mx-auto stroke-red-600' />
                </button>
                {image ? (
                  <img
                    src={image}
      ...
      <button
        type='submit'
        className='w-full p-3 bg-blue-500 hover:bg-blue-600 text-white rounded-lg transition-colors flex items-center justify-center mt-4'
      >
        Submit
        {isLoading && (
