...
const COLS = [
  ...
  { key: "customerMobiles", label: "Customer mobile" },
  ...
  { key: "botMessage", label: "Bot msg" },
  { key: "interested", label: "Interested" },
  { key: "withFacebook", label: "FB link" },
  { key: "withMessenger", label: "Msg link" },
];
...
export const TeamSummaryTable = ({
  ...
  <table className="min-w-full text-sm">
      ...
      <tr className="text-xs uppercase tracking-wider text-gray-600">
        <th className="px-3 py-2 text-left sticky left-0 z-10 bg-gray-50 min-w-[9rem]">
          <button type="button" onClick={() => toggleEmployeeSort("name")} className="inline-flex items-center gap-1">
            Team <span className="text-[10px]">{getEmployeeSortIndicator("name")}</span>
          </button>
        </th>
        {COLS.map(({ key, label }) => (
          <th key={key} className="px-3 py-2 text-center whitespace-nowrap">
    ...
    <tbody className="divide-y divide-gray-100">
      ...
      {!loading && pagedTeamRows.length === 0 && (
        <tr>
          <td className="px-3 py-8 text-center text-gray-500" colSpan={EMPLOYEE_ACTIVITY_COLUMN_COUNT}>
            {filteredRows.length === 0
              ...
              : "No team names were found on the filtered records."}
          </td>
        </tr>
      )}
      {loading && (
    ...
    {!loading && sortedTeamRows.length > 0 && (
