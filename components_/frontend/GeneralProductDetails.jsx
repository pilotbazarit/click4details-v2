...
import GeneralProductDetailsDescription from "./GeneralProductDetailsDescription";
...
const GeneralProductDetails = ({ productDetails }) => {
    ...
    const [selectedVariation, setSelectedVariation] = useState(productDetails?.prices[0] || '');
    const [selectedPrice, setSelectedPrice] = useState(productDetails?.prices[0]?.pp_discount_price || 'Call for Price');

    const handleVariationClick = (variation) => {
        setSelectedVariation(variation);
        setSelectedPrice(variation.pp_discount_price || 'Call for Price');
    };

    const handleCopy = (e) => {
        e.preventDefault();
        if (productDetails?.p_code) {
            // console.log("Original v_code:", productDetails.p_code);

            const cleanedCode = productDetails.p_code.replace(/^[^-]*-/, "");
    ...
    useEffect(() => {
        if (productDetails) {
            const sliderImages = [];
            if (productDetails?.p_images && productDetails?.p_images.length > 0) {
                productDetails.p_images.map((img) => {
    ...
    const handleCopyClick = async () => {
        ...
        if (productDetails?.brand) {
            detailsToCopy += `Brand : ${productDetails?.brand?.md_title}\n`;
        }
        // Model
        if (productDetails?.model) {
            detailsToCopy += `Model: ${productDetails?.model?.md_title}\n`;
        }
        // // Package
    ...
    const handleImageShare = async () => {
        ...
        const productUrl = `${domain}/general-product/${productDetails.p_slug}`;
        const text = `Check out this product: ${productUrl}`;
        const title = productDetails?.p_name || 'Product Images';
            ...
            try {
                ...
                if (error.name !== 'AbortError') {
                    ...
                    try {
                        ...
                        const messageText = `${productDetails?.p_name}\nBrand : ${productDetails?.brand?.md_title}\nModel: ${productDetails?.model?.md_title}\n \nFor more pictures and details, please browse our website link provided below: ${productUrl}`;
        ...
        <div className="px-4">
            <div>
                <div className="flex flex-col-reverse md:flex-row md:justify-between md:items-center mt-4">
                    <div>
                        <p className="text-xl font-bold md:text-3xl md:font-medium">{productDetails?.p_name}</p>
                        <span className="text-gray-500">{dayjs(productDetails?.p_created_at).fromNow()}</span>
                    </div>
            ...
            <div className="mt-4 grid grid-cols-1 md:grid-cols-5 gap-2 mb-6">
                <div className="md:col-span-3">
                                ...
                                <div className="space-y-2">
                                        ...
                                        <div className="space-y-2">
                                            <div>
                                                <div className="mt-2 flex flex-wrap gap-2">
                                                    {productDetails?.prices && productDetails.prices.map((v, i) => (
                                                        <button
                                                            key={v.id || i} // key হিসেবে ইউনিক id ব্যবহার করা ভালো, না থাকলে index ব্যবহার করুন
                                                            onClick={() => handleVariationClick(v)}
                                                            className={`px-4 py-2 text-sm font-medium border rounded-md transition-colors ${selectedVariation?.unit?.md_id === v.unit?.md_id
                                                                ? 'bg-blue-600 text-white border-blue-600' // Active button style
                                                                : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50' // Inactive button style
                                                                }`}
                                                        >
                                                            {v?.unit?.md_title}
                                                        </button>
                                            ...
                                            <div className="font-bold text-gray-600 text-md md:text-xl">
                                                TK. {formatPrice(selectedPrice)}
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex space-x-4 items-center">
                                        <div className="flex items-center">
                                            ...
                                            >
                                                {copied ? "Copied!" : productDetails?.p_code}
                                            </span>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div className="mt-4 hidden md:inline">
                        {/* dsgf sdfg dfgs dfgssdgdfgssdfgsdfgsdfg */}
                        <GeneralProductDetailsDescription productDetails={productDetails} basePath={basePath} />
                    </div>
                    {/* // )} */}
                </div>
                ...
                <div className="md:col-span-2 md:col-start-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-1 xl:grid-cols-2 md:gap-x-1 lg:gap-x-2 text-sm">
                            <div className="">
                                <div className="grid grid-cols-6 gap-2">
                                    ...
                                    <div className="col-span-3 text-base font-semibold">{productDetails?.brand?.md_title}</div>
                                </div>
                                <div className="grid grid-cols-6 gap-2">
                                    <div className="col-span-3 text-base">Model:</div>
                                    <div className="col-span-3 text-base font-semibold">{productDetails?.model?.md_title}</div>
                                </div>
                                <div className="grid grid-cols-6 gap-2">
                                    <div className="col-span-3 text-base">Category:</div>
                                    <div className="col-span-3 text-base font-semibold">{productDetails?.category?.c_name}</div>
                                </div>

                                {
                                    productDetails?.extended_data?.length > 0 && productDetails?.extended_data.map((extraData, index) => (
                                        <div className="grid grid-cols-6 gap-2" key={index}>
                                            <div className="col-span-3 text-base">{extraData?.ed_entity_key}:</div>
                                            <div className="col-span-3 text-base font-semibold">{extraData?.ed_entity_value}</div>
                                        </div>
                                    ))
                                }
                    ...
                    <div className="border rounded shadow-sm p-4 mt-4">
                        <div className=" mb-4 border-b pb-2 flex items-center justify-between">
                            <h2 className="text-lg font-medium text-blue-600">Attribue</h2>
                            {/* এখানে onClick ইভেন্ট যোগ করা হয়েছে */}
                            {/* <button className="text-lg font-medium text-blue-600 flex items-center gap-1" onClick={handleCopyClick}>
                                <Copy /> Copy
                            </button> */}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-1 xl:grid-cols-2 md:gap-x-1 lg:gap-x-2 text-sm">
                            <div className="">
                                ...
                                {
                                    productDetails?.attributes?.length > 0 && productDetails?.attributes.map((attr, index) => (
                                        <div className="grid grid-cols-6 gap-2" key={index}>
                                            <div className="col-span-3 text-base">{attr?.attribute?.md_title}:</div>
                                            <div className="col-span-3 text-base font-semibold">{attr?.attribute_value?.fs_title}</div>
                                        </div>
                                    ))
                                }
            ...
            {!user && (
                <div className="mt-4 inline md:hidden">
                    <GeneralProductDetailsDescription productDetails={productDetails} />
                </div>
            )}
        </div>
    );
