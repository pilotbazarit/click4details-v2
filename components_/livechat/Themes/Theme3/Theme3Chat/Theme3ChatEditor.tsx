...
export default function Theme3ChatEditor({
  editorHeight,
  setEditorHeight
}: {
  editorHeight: number;
  ...
  const theme3EditorRef = useRef<HTMLDivElement>(null);
  ...
  const handleInput = () => {
    const text = theme3EditorRef?.current?.innerText;
    setInputtedText(text?.trim() || '');
    if (theme3EditorRef?.current?.scrollHeight) {
      const replyHeight = selectedMessageToReply?.id
        ? replyRef?.current?.scrollHeight || 0
        : 0;
      const scrollHeight =
        theme3EditorRef.current?.scrollHeight + 38 + replyHeight;
      ...
      if (theme3EditorRef.current) {
        theme3EditorRef.current.style.setProperty(
          'height',
          !text?.trim()
            ? '50px'
            : theme3EditorRef.current.scrollHeight > 120
            ? '120px'
            : `${theme3EditorRef.current.scrollHeight + 2}px`,
          'important'
        );
      }
    }
    ...
    if (actAsGuest) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (theme3EditorRef.current) {
            theme3EditorRef.current.innerText = '';
  ...
  const handleEmojiClick = (emojiData: any) => {
    ...
    if (theme3EditorRef?.current) {
      theme3EditorRef.current.innerText = text;
    }
  };
      ...
      if (theme3EditorRef.current) {
        theme3EditorRef.current.style.setProperty(
          ...
          !selectedMessageToReply?.id
            ? theme3EditorRef.current.scrollHeight > 120
              ? '120px'
              : theme3EditorRef.current.scrollHeight < 50
              ? '50px'
              : `${theme3EditorRef.current.scrollHeight + 2}px`
            : theme3EditorRef.current.scrollHeight > 120
            ? '120px'
  ...
  return (
    <div
      className={`px-4 pt-2 pb-0 border-t ${
        actAsGuest && (!isAgentContinued || isClosed) ? 'hidden' : ''
      }`}
    >
      <form
        onSubmit={async (e) => {
      ...
      >
        {attachments?.length > 0 && (
          <div className='max-h-[150px] overflow-auto absolute bottom-[150px] left-[-16px] bg-[#f8f9fa] border border-slate-700 rounded-lg m-5 p-2 w-[calc(100%-16px)] scrollbar'>
        ...
        <div
          className={`flex gap-2 ${
            (theme3EditorRef?.current?.scrollHeight &&
              theme3EditorRef?.current?.scrollHeight > 48) ||
            selectedMessageToReply?.id
              ? 'items-end'
              : 'items-center'
          }`}
        >
          <div className='flex-1 relative'>
            ...
            <div className={isRecording ? 'opacity-0 relative' : ''}>
              <div
                ref={theme3EditorRef}
                className='flex-1 p-3 border border-gray-200 outline-none relative whitespace-pre-wrap break-words sm:w-full overflow-auto scrollbar rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 h-[50px] max-w-[306px] sm:max-w-[366px]'
              ...
              {inputtedText?.trim()?.length === 0 && (
                <span className='absolute bottom-[50px] left-3 text-gray-400 pointer-events-none'>
                  Message...
                </span>
              )}
            </div>
            <canvas
              ...
              height={50}
              className={
                isRecording ? 'block rounded-lg absolute bottom-9' : 'hidden'
              }
            />
            {isRecording && (
              <button
                className='absolute top-0 rounded-l-md left-0 outline-none mx-auto cursor-pointer bg-[#1e293b] px-4 py-[15px]'
                type='button'
                onClick={() => {
            ...
            <div className='flex justify-end items-center gap-2'>
              ...
              >
                <Smile className='w-5 h-5 stroke-gray-400 hover:stroke-blue-600' />
              </button>
              {showEmojiPicker && (
                <div
                  className='fixed sm:bottom-[170px] bottom-[155px] right-1 sm:right-[40px] mb-2'
                  ref={emojiPickerRef}
                >
                  <Picker
              ...
              >
                <Paperclip
                  size={20}
                  className='stroke-gray-400 hover:stroke-blue-600'
                />
              </button>
              <button
                type='button'
              ...
              >
                <Mic
                  ...
                  className={`hover:stroke-blue-600 ${
                    isRecording ? 'stroke-red-500' : 'stroke-gray-400'
                  }`}
                />
              </button>
              <button
              ...
              >
                <Send
                  size={20}
                  className='stroke-blue-600 hover:stroke-blue-700'
                />
              </button>
            </div>
          </div>
